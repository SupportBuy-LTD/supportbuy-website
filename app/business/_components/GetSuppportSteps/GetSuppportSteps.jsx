  ...
  return (
    <section className=" lg:flex gap-10 items-center mt-10  bg-sb-white-two rounded-2xl px-6 py-6 md:px-20 md:py-[200px] shadow-[0px_1px_4px_0px_#0C0C0D0D]">
      <div>
        ...
        <div className="hidden lg:block pt-6 w-full max-w-[424px]">
          <Image
            src="/support_buy_merchant_dashboard.gif"
            alt="SupportBuy merchant dashboard"
            width={384}
            height={192}
            className="h-auto w-full rounded-lg border border-sb-primary/10 object-contain shadow-[0px_8px_24px_0px_#0C0C0D14]"
          />
        </div>
      </div>
      <div className="lg:gap-[88.5px] mt-10 py-[33.5px] w-full max-w-[490px] ">
        {BUSINESSSTEPSDATA.map(
          (item) => (
            <Steps
              key={item.id}
              id={item.id}
